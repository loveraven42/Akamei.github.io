...
# 10 GB disk space for collect GraphicsMagick output files ..
MAX_OUTPUT_FILES_DISK_SPACE = 10 * 1024 * 1024 * 1024
MAX_PROCESS_WAIT_TIME = 10  # wait time for process exit ..


def run_graphicsmagick_convert(input_file, output_file):
    """
    推理得知，依靠ASAN 和 自己开发的fuzz数据，得到的crash然后解析成基本文件
    最后得到  cve
    我也想得到cve:)
    """
    process = subprocess.Popen(['./magick', 'convert', input_file, output_file], stdout=subprocess.PIPE,
                               stderr=subprocess.PIPE)
    # 这两个函数写法挺有趣的

    def process_timeout_exit(): return process.kill()
    timeout = threading.Timer(MAX_PROCESS_WAIT_TIME, process_timeout_exit)
    timeout.start()

    # 去了解一下subprocesss.Popen这个函数如何实现的
    # 猜测是运行程序，然后判
    process.wait()

    timeout.cancel()
    # 这种写法判断了函数是否运行时间超过的设定的最大时间
    # 这种写法可以学习一下

    # 得到输出
    std_error_output = process.stderr.read()

    # 我们刚开始调试的话最好就先进行到这步，然后在接下来做其他的事情
    print(type(std_error_output))
    print(std_error_output)

    # 这里看起来总是有点瑕疵， 毕竟可以写成 and std_error_output.find('=======') != -1
    if len(std_error_output) and not -1 == std_error_output.find('========='):
        flag_address_santize = "ERROR:AddressSantizer"
        flag_address_santize_offset = std_error_output.find(
            flag_address_santize)
        flag_leak_sanitize = "ERROR: LeakSanitizer"
        flag_leak_sanitize_offset = std_error_output.find(flag_leak_sanitize)

        if not -1 == flag_address_santize:
            # 获取flag_address_santize的
            crash_type = std_error_output[flag_address_santize_offset +
                                          len(flag_address_santize):]
            ...
            # 这里得到crash的pc的相关信息
            crash_point = std_error_output[flag_at_pc_offset +
                                           len(flag_at_pc):]
            crash_point = crash_point[: crash_point.find(" bp")]

            # 然后对相关信息以及crash进行解析
            # 分别列出哪些类型, 哪些类型
            if not -1 == crash_type.find("buufer-overflow"):  # stack and heep
                flag_of_size = ' of size'
                flag_of_size = std_error_output.find(flag_of_size)

                crash_type_detail = std_error_output[: flag_at_pc_offset]
                crash_type_detail = crash_type_detail[crash_type_detail.rfind(
                    "\n"):].strip()
            elif 'SEGV' == crash_type:  # 空指针错误
                flag_of_addresss = "unknown address"
                flag_of_addresss_offset = std_error_output.find(
                    flag_of_addresss)

                crash_type_detail = std_error_output[flag_of_addresss_offset + len(
                    flag_of_addresss):]
                crash_type_detail = crash_type_detail[: crash_type_detail.find(
                    '(')].strip()
            return True, crash_type, crash_type_detail, crash_point, std_error_output
        elif not -1 == flag_leak_sanitize:
            flag_direct_leak = 'Direct leak'
            flag_indirect_leak = 'Indirect leak'
            memory_leak_id = std_error_output.count(
                flag_direct_leak) + std_error_output.count(flag_indirect_leak)
...
def copy_all_file(src_dir, dest_dir):
    ...
    for dir_index in dir_list:
        ...
        if max_file_space < MAX_OUTPUT_FILES_DISK_SPACE:
            # os.system('cp ' + file_path + ' ' + dest_dir)
            copy_file(file_path, dest_dir + '/' + dir_index)
        else:
            break
...
if __name__ == '__main__':
    if 2 == len(sys.argv):
        ...
        if os.path.exists(input_file_dir):
            ...
            now_time_string = str(now_time.tm_year) + '_' + str(now_time.tm_mon) + '_' + str(
                now_time.tm_mday) + '_' + str(now_time.tm_hour) + '_' + str(now_time.tm_min) + '_' + str(now_time.tm_sec)
            ...
            # 以上代码 是在输入的目录里 创建 input ,output, crash目录

            if os.path.isdir(input_file_dir):
                copy_all_file(input_file_dir, crash_dir_crash_input_dir)
            elif os.path.isfile(input_file_dir):
                # os.system('cp ' + input_file_dir + ' ' + crash_dir_crash_input_dir)
                copy_file(input_file_dir,
                          crash_dir_crash_input_dir + '/sample')
            ...
            while True:  # dead loop ..
                ...
                # calcute output files space for save disk space ..
                output_file_space = 0
                graphicsmagick_input_file_list = os.listdir(
                    crash_dir_crash_input_dir)
                current_fuzzing_loop_test_files_count = len(
                    graphicsmagick_input_file_list) * len(imagemagick_output_format)

                print_output('Fuzzing Write Loop ' + str(loop_index) +
                             ' Start ,Sample Files ' + str(len(graphicsmagick_input_file_list)))
                ...
                for graphicsmagick_input_file_index in graphicsmagick_input_file_list:
                    graphicsmagick_input_file_index_path = crash_dir_crash_input_dir + \
                        '/' + graphicsmagick_input_file_index
                    # this flag will exit fuzzing ..
                    disk_space_save_signal = False

                    for graphicsmagick_output_format_index in imagemagick_output_format:
                        graphicsmagick_output = crash_dir_crash_output_dir + '/' + \
                            graphicsmagick_input_file_index + '_' + graphicsmagick_output_format_index
                        graphicsmagick_output_extern_name = get_extern_name(
                            graphicsmagick_output)

                        # + ' Input:' + graphicsmagick_input_file_index_path + ' Output Format:' + graphicsmagick_output_extern_name + '  ')
                        print_progress(test_index / (current_fuzzing_loop_test_files_count * 1.0), str(
                            test_index) + '/' + str(current_fuzzing_loop_test_files_count))

                        # it will be block by GraphicsMagick Dead-Loop
                        result = run_graphicsmagick_convert(
                            graphicsmagick_input_file_index_path, graphicsmagick_output)
                        ...
                        if result[0]:  # catch crash
                            ...
                            if not None == result[1]:
                                if 'SEGV' == result[1] or 'Memory-Leak' == result[1]:
                                    ...
                                    crash_file_path = result[1] + '-' + result[2] + '_output_' + \
                                        graphicsmagick_output_extern_name + \
                                        '_' + str(time.time())
                                else:  # crash overflow ..
                                    if check_exist_crash(crash_dir_crash_sample_dir, result[1], result[2] + '-' + result[3]):
                                        continue

                                    crash_file_path = result[1] + '-' + result[2] + '-' + result[3] + \
                                        '_output_' + graphicsmagick_output_extern_name + \
                                        '_' + str(time.time())

                                print_output('Crash (' + str(time.time()) + ') ' +
                                             result[1] + ' ' + result[2] + ' ' + graphicsmagick_output_extern_name)
                            else:
                                crash_file_path = 'unknow_crash_output_' + \
                                    graphicsmagick_output_extern_name + \
                                    '_' + str(time.time())

                                print_output(
                                    'Crash (' + str(time.time()) + ') unknow crash' + graphicsmagick_output_extern_name)
                            ...
                            # os.system('cp ' + graphicsmagick_input_file_index_path + ' ' + crash_dir_crash_sample_dir + '/' + crash_file_path)
                            copy_file(graphicsmagick_input_file_index_path,
                                      crash_dir_crash_sample_dir + '/' + crash_file_path)
                        ...
                        try:
                            output_file_space += os.path.getsize(
                                graphicsmagick_output)
                        except:
                            pass
                ...
                copy_all_file(crash_dir_crash_output_dir,
                              crash_dir_crash_input_dir)
                delete_all_file(crash_dir_crash_output_dir)

                print_output('Fuzzing Write Loop ' + str(loop_index) +
                             ' Exit ,Get Crash :' + str(crash_index))
        else:
